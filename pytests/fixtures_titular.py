...
def titular(fecha_temprana: date) -> Titular:
    return Titular.crear(sk='titular', nombre='Titular', fecha_alta=fecha_temprana)


@pytest.fixture
def otro_titular(fecha: date) -> Titular:
    return Titular.crear(sk='otro', nombre='Otro Titular', fecha_alta=fecha)


@pytest.fixture
def titular_gordo(fecha: date) -> Titular:
    return Titular.crear(sk='gordo', nombre='Titular Gordo', fecha_alta=fecha)

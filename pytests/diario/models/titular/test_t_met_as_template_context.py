...
@pytest.fixture
def context(titular):
    return titular.as_template_context()
...
def test_cuentas_del_titular_se_ordenan_por_nombre(titular, cuenta_2, cuenta):
    context = titular.as_template_context(es_elemento_principal=True)
    assert \
...
def test_si_recibe_movimiento_incluye_saldo_historico_de_cuentas_del_titular(
    ...
    print("Todavía no se configuró correctamente la clave saldo en Cuenta.as_template_context()")
    titular = cuenta.titular
    context = titular.as_template_context(entrada)
    assert \
        [x['saldo'] for x in context['cuentas']] == \
        [x.as_template_context(entrada)['saldo'] for x in (cuenta, cuenta_2)]
...
def test_si_es_elemento_principal_incluye_cuentas_del_titular(
    ...
    assert \
        context['cuentas'] == \
        [cuenta.as_template_context(), cuenta_2.as_template_context()]


def test_si_no_es_elemento_principal_no_incluye_saldo_gral(titular):
    context = titular.as_template_context(es_elemento_principal=False)

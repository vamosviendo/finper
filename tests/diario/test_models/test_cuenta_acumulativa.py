...
class TestSubcuentas(TestCase):
    ...
    def test_si_se_elimina_mov_con_subcuenta_como_cta_entrada_en_fecha_con_otro_movimiento_en_subcuenta_se_resta_importe_de_saldo_de_cta_madre_en_fecha(self):
        mov = Movimiento.crear('mov subc', 45, self.cta2, fecha=date(2019, 1, 10))
        Movimiento.crear('otro mov', 40, self.cta2, fecha=date(2019, 1, 10))
        saldo = Saldo.tomar(cuenta=self.cta1, fecha=date(2019, 1, 10)).importe
        ...
        self.assertEqual(
            self.cta1.saldo_set.get(fecha=date(2019, 1, 10)).importe,
            saldo-45
        )
    ...
    def test_si_se_elimina_mov_con_subcuenta_como_cta_salida_en_fecha_con_otro_movimiento_en_subcuenta_se_suma_importe_al_saldo_de_cta_madre_en_fecha(self):
        mov = Movimiento.crear('mov subc', 45, None, self.cta2, fecha=date(2019, 1, 10))
        Movimiento.crear('otro mov', 40, self.cta2, fecha=date(2019, 1, 10))
        saldo = Saldo.tomar(cuenta=self.cta1, fecha=date(2019, 1, 10)).importe
    ...
    def test_si_se_elimina_unico_mov_con_subcuenta_como_cta_entrada_en_fecha_con_movimiento_de_otra_subcuenta_se_resta_importe_de_mov_de_saldo_de_cta_madre_en_fecha(self):
        mov = Movimiento.crear('mov subc', 45, self.cta2, fecha=date(2019, 1, 10))
        Movimiento.crear('otro mov', 40, self.cta3, fecha=date(2019, 1, 10))
        saldo = Saldo.tomar(cuenta=self.cta1, fecha=date(2019, 1, 10)).importe
    ...
    def test_si_se_elimina_unico_mov_con_subcuenta_como_cta_entrada_en_fecha_sin_movimientos_de_otra_subcuenta_de_cta_madre_se_elimina_saldo_de_cta_madre_en_fecha(self):
        ...
        with self.assertRaises(Saldo.DoesNotExist):
            self.cta2.saldo_set.get(fecha=date(2019, 1, 10))

        with self.assertRaises(Saldo.DoesNotExist):
            self.cta1.saldo_set.get(fecha=date(2019, 1, 10))
    ...
    def test_si_se_elimina_mov_de_subcuenta_no_unico_en_fecha_con_movimientos_de_otra_subcuenta_de_cta_abuela_con_movimientos_en_fecha_se_resta_importe_de_saldo_de_cta_abuela(self):
        fecha = date(2019, 1, 10)
        subc21, subc22 = self.cta2.dividir_entre(
            ['subcuenta 2.1', 'sc21', 0],
            ['subcuenta 2.2', 'sc22'],
            fecha=date(2019, 1, 5)
        )
        mov = Movimiento.crear('movsubsubc 1', 40, subc21, fecha=fecha)
        Movimiento.crear('movsubsubc 2', 30, subc21, fecha=fecha)
        Movimiento.crear('mov subc', 20, self.cta3, fecha=fecha)
        saldo = Saldo.tomar(cuenta=self.cta1, fecha=date(2019, 1, 10)).importe

        mov.delete()

        self.assertEqual(
            self.cta1.saldo_set.get(fecha=fecha).importe,
            saldo-40
        )
    ...
    def test_si_se_elimina_unico_mov_de_subcuenta_en_fecha_con_movimientos_de_otra_subcuenta_de_cta_abuela_se_resta_importe_de_saldo_de_cta_abuela_en_fecha(self):
        ...
        mov = Movimiento.crear('mov subsubc', 40, subc21, fecha=fecha)
        Movimiento.crear('mov subc', 30, self.cta3, fecha=fecha)
        saldo = Saldo.tomar(cuenta=self.cta1, fecha=fecha).importe

        mov.delete()

        with self.assertRaises(Saldo.DoesNotExist):
            subc21.saldo_set.get(fecha=fecha)

        with self.assertRaises(Saldo.DoesNotExist):
            self.cta2.saldo_set.get(fecha=fecha)
